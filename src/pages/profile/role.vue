    ...
    <div class="maxmd:w-screen ">
      <div class="text-[#333437] text-3xl font-medium flex justify-between">
        ...
        <button
            ...
            @click="show_add = true"
            class="hover:bg-primary hover:text-white duration-300 transition-all rounded-xl border border-[#DBDBDB] text-l_white text-xl py-2 px-4">{{
            $t('Добавить роль')
          }}
        </button>
      </div>
        ...
        <div class="px-4 py-4 rounded-2xl overflow-x-hidden">
          ...
          <div class="flex gap-x-4 mt-4 justify-center">
            <button @click="show_del = false"
                    class="bg-[#F7F8F9] hover:bg-primary_gr hover:text-white transition-all duration-300 rounded-xl border border-[#DBDBDB] text-[#8A92A6] text-2xl maxsm:text-[14px] py-2 px-16 maxsm:px-10">
              {{ $t('Отменить') }}
            </button>
            <button @click="del_role({role_id:current_role.id});show_del = false"
                    class="hover:bg-danger hover:text-white transition-all duration-300 rounded-xl border border-[#DBDBDB] text-l_white text-2xl maxsm:text-[14px] py-2 px-16 maxsm:px-10">
              {{ $t('Удалить') }}
            </button>
          </div>
        </div>
