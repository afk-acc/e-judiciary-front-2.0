  ...
  <div class="" v-if="!(item.deleted) ">
    <div class="flex flex-col gap-y-2 relative border border-filter_gray border-opacity-30 p-2">
      ...
      <div class="flex gap-x-4 max-sm:mt-4 items-center">
        ...
        <input type="text"
               required
               class="outline-none p-2 rounded-md max-sm:w-full border border-filter_gray focus:border-borderFocus focus:shadow-inputFocus" v-model="item.name_uz_l">
      </div>
      <div class="gap-x-4 flex items-center">
        {{ $t("Уз") }}
        <input type="text"
               required
               class="outline-none p-2 rounded-md max-sm:w-full border border-filter_gray focus:border-borderFocus focus:shadow-inputFocus" v-model="item.name_uz_c">
      </div>
      <div class="mt-10 mb-10">
        <div class="font-medium">{{ $t("Тип поля") }}</div>
        <div class="">
          <select name="" id="" class="outline-none p-2 border focus:border-borderFocus focus:shadow-inputFocus border-filter_gray rounded-md"
                  required
        ...
        <div class="" v-if="Number(item.input_type_id) === 2">
          ...
          <div class="pl-10 max-sm:pl-0 flex flex-col max-sm:items-center gap-y-2">
            <div class="mx-auto" v-for="(val,index) in list.uz_l" :key="index">
              <input type="text" class="p-2 outline-none rounded-md border border-filter_gray focus:border-borderFocus focus:shadow-inputFocus"
                     required
            ...
            <div class="my-2">
              <button type="button" class="bg-[#007bff] hover:bg-[#0069d9] hover:border-[#0062cc] transition-all duration-300 rounded-md p-2 text-white" @click="list.uz_l.push('')">{{
                  $t('Добавить значение')
                }}
              </button>
            </div>
