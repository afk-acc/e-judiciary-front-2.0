<template>
  <div class="bg-white w-full px-10  pb-5 rounded-lg max-md:py-4 relative z-100">
    ...
    <div class="flex max-lg:flex-wrap  max-lg:mt-10 ">
      <input class="bg-footer_bg w-full  text-sm rounded-xl px-4 py-2 text-[#9DA3CF] outline-none"
             @keyup.enter.tab="loadHistoryLists({...params, query:$event.target.value})" type="text" name="" value=""
             :placeholder="$t('Поиск изменений')">
      <button
          class="text-sm max-lg:w-full max-lg:px-0 max-lg:mt-4 bg-footer_bg ml-10 max-lg:m-0 px-6 py-4 rounded-xl outline-none"
          type="submit"
    ...
    <div
        class="w-[394px] max-sm:w-[90%] shadow-2xl text-sm border-filter_gray bg-white absolute right-10 max-sm:right-4  z-30 p-4"
      ...
      <div class="filter-body mt-5">
        <div>
          ...
          <ul class="text-sm text-filter_gray flex justify-around mx-auto text-center mt-[6px]">
            <li class="w-6/12 py-2 rounded-xl " @click="par.category = 'all'"
                :class="{'bg-primary_gr text-white ':par.category === 'all'}">
              {{ $t("Все") }}
            </li>
            <li class="w-6/12 py-2 ml-4 rounded-xl" @click="par.category = 'login'"
                :class="{'bg-primary_gr text-white ':par.category === 'login'}">
              {{ $t("Авторизация") }}
            </li>
          </ul>
          <ul class="text-base text-filter_gray flex justify-around mx-auto text-center mt-5">
            <li class="w-6/12 py-2 rounded-xl" @click="par.category = 'add'"
                :class="{'bg-primary_gr text-white ':par.category === 'add'}">
              {{ $t("Изменение") }}
            </li>
            <li class="w-6/12 py-2 ml-4 rounded-xl" @click="par.category = 'delete'"
                :class="{'bg-primary_gr text-white ':par.category === 'delete'}">
              {{ $t("Удаления") }}
            </li>
          </ul>
        </div>
        <div class="mt-8">
          ...
          <ul class="text-sm text-filter_gray flex items-center justify-around mx-auto text-center mt-[6px]">
            <li class="w-6/12 py-2 rounded-xl" @click="par.sort='asc'"
                :class="{'bg-primary_gr text-white ':par.sort === 'asc'}">
              {{ $t("Возрастанию") }}
            </li>
            <li class="w-6/12 py-2 ml-4 rounded-xl" @click="par.sort='desc'"
                :class="{'bg-primary_gr text-white ':par.sort === 'desc'}">
              {{ $t("Убыванию") }}
            </li>
          </ul>
          <p class="text-filter_gray mt-5">{{ $t("Дата:") }}</p>
          <ul class="text-sm text-filter_gray mt-[6px]">
            <input v-model="par.date" type="date"
                   class="w-6/12 py-2 ml-4 rounded-xl text-center outline-none border-[#E9ECEF] border-[1px] "
                   placeholder="дд.мм.гг"/>
          </ul>
          <div class="flex w-full my-2 justify-center">
            <button class="rounded-2xl bg-primary text-white px-4 py-2 w-9/12"
                    @click="loadHistoryLists({...params, ...par})">
              {{ $t("Применить") }}
            </button>
