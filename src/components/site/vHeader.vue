  ...
  <teleport to="body">
    <div
      ...
      'bg-header_gr': showBg() ,
        ...
        class="fixed  max-lg:bg-primary  transition-all duration-300 top-0 left-0 w-full border-b-[0.5px] border-white border-opacity-40 h-[82px] z-50">
      <div class="_container flex  max-lg:flex-col items-center justify-between relative h-full">
        <div class="flex max-lg:w-9/12  h-full max-lg:w-10/12 items-start pt-2">
         <router-link to="/" class=" h-[50px]  relative  ">
        ...
        <div class="flex transition-all max-lg:absolute max-lg:bg-primary max-lg:w-full max-lg:flex-col items-center justify-between"
        :class="{'max-lg:top-[-500px]' : !openBurger, 'max-lg:top-[80px]' : openBurger}">
          <ul class="flex max-lg:flex-col gap-x-8 text-white font-bold mr-4 max-lg:text-3xl items-center">
            <router-link :to="{name:'service', params:{page:1}}" class="cursor-pointer py-10">{{ $t('Услуги') }}</router-link>
                    <li class="cursor-pointer py-10">{{ $t('Вопросы') }}</li>
            <router-link :to="{name:'lawyers', params:{page:1}}" class="cursor-pointer py-10">{{ $t('Юристы') }}</router-link>
            <li class="cursor-pointer py-10">{{ $t('Блог') }}</li>
            <li class="cursor-pointer py-10">{{ $t('Контакты') }}</li>
        ...
        <div class="flex  max-lg:flex-col gap-x-8 text-white items-center ">
          ...
          <div class="flex max-lg:flex-col gap-x-4 items-center" v-if="show()">
            <div class="header-icons flex max-lg:flex-col  items-center ">
              <div class="notification flex max-lg:flex-col items-center">
                ...
                <router-link :to="{name:'info'}" class="flex gap-x-4 max-lg:my-6 items-center">
  ...
  methods: {
    ...
    showBg() {
      if (this.$route.name === 'main')
        return this.scrollPosition > 380
      return true
    }
  },
  mounted() {
